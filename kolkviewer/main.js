...
class KolkturmViewer {
    ...
    static vw = null; // View (canvas) width
    static vh = null; //   and height
    static vr = null; // View ratio
    static iw = null; // Image width
    static ih = null; //   and height
    static posx = 0; // X and
    static posy = 0; //   y position of the clipping area
    static posz = 1.0; // Z position (zoom factor)
    static minz = null; // Minimum z position
    static nx_pos = null; // The position at the x axis where north is located
    static mousex = null;
    static mousey = null;
     ...
     */
    static animation_running = false;

    // Public variables required for the compass
    static compass_path2d = [];
    px_letter_circle = [];
    static py_letter_circle = [];

    static objects = [];
    static last_hovered_object = null;
    static objinfo_div = null;
    static draw_all_object_areas = false;
    static draw_object_areas_on_mouseover = false;
    ...
    constructor(data_url) {
        ...
        $(this.canvas).mousewheel(mouseWheelHandler);
        $(this.canvas).mousedown(mouseDownHandler);
        $(this.canvas).mousemove(mouseHoverObjectHandler);
    ...
    mouseMoveHandler(event) {
        if (mouseclicked) {
            if (mousex != null && mousey != null) {
                var deltax = (mousex - event.pageX)*(this.ih*this.posz/this.vh);
                var deltay = (mousey - event.pageY)*(this.ih*this.posz/this.vh);
                movex(deltax);
                movey(deltay);
                draw();
            }
    ...
    mouseWheelHandler(event) {
        zoom(event.deltaY*event.deltaFactor/-10000, event.pageX/this.vw, event.pageY/this.vh);
        draw();
    }
    ...
    zoom(zi, fx, fy) {
        ...
        movex((oldz*this.vr*this.ih - this.posz*this.vr*this.ih)*fx);
        movey((oldz*this.ih - this.posz*this.ih)*fy);
    }
    ...
    resetZoom() {
        ...
        movex((oldz*this.vr*this.ih - this.vr*this.ih)/2); // Keep center centered while reset zoom factor
    }
    ...
    drawCompass(x, y, d) {
        ...
        for (var i = 0; i < compass_path2d.length; i++) {
            ...
            this.ctx2d.fill(compass_path2d[i]);
        }

        this.ctx2d.font = '12px serif';
        this.ctx2d.fillStyle = 'red';
        this.ctx2d.fillText('N', this.px_letter_circle[2] - 5, py_letter_circle[2]);
        this.ctx2d.fillStyle = 'black';
        this.ctx2d.fillText('S', this.px_letter_circle[0] - 4, py_letter_circle[0] + 9);
        this.ctx2d.fillText('O', this.px_letter_circle[1] - 2, py_letter_circle[1] + 5);
        this.ctx2d.fillText('W', this.px_letter_circle[3] - 10, py_letter_circle[3] + 5);
